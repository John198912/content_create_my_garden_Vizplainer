...
#     <path-to>/themes/all-themes.css <id> <project>/src/styles/tokens.css
#
# 存储模型：所有主题合并在 themes/all-themes.css（[data-theme="<id>"] 选择器）
# + themes/all-themes-meta.json（主题元数据）。脚手架时用 extract-theme.mjs
# 把选定主题的块展开成 :root 写到产物的 tokens.css —— 产物结构不变。
# ─────────────────────────────────────────────────────────────
set -euo pipefail
...
THEMES_CSS="$THEMES_DIR/all-themes.css"
THEMES_META="$THEMES_DIR/all-themes-meta.json"
EXTRACT="$SKILL_DIR/scripts/extract-theme.mjs"
DEFAULT_THEME="midnight-press"

list_themes() {
  echo "可用主题（来自 ${THEMES_CSS}）:"
  echo
  node "$EXTRACT" --list "$THEMES_META"
...
if [[ ! -f "$THEMES_CSS" ]]; then
  echo "✗ 找不到合并主题文件：$THEMES_CSS" >&2
  exit 1
fi
# 校验主题 id 是否存在于合并文件中（grep 选择器即可）
if ! grep -q "\[data-theme=\"${THEME}\"\]" "$THEMES_CSS"; then
  echo "✗ 找不到主题 '${THEME}'。可用主题：" >&2
  echo >&2
  grep -oE '\[data-theme="[^"]+"\]' "$THEMES_CSS" | sed -E 's/.*"([^"]+)".*/    • \1/' >&2
  exit 1
fi
...
# tokens.css 来自所选主题 —— 从合并文件抽块展开成 :root
node "$EXTRACT" "$THEMES_CSS" "$THEME" src/styles/tokens.css
...
写章节时必读（单一入口，路径在 SKILL 仓库内）：
  ...
  • $SKILL_DIR/themes/all-themes-meta.json（查 "$THEME" 这条）
      看 descriptionZh / mood / bestFor —— 参考主题气质
      （动画 / 时长 / 字号 / emoji 由 chapter agent 在每章自由决定）
      列全部：node $SKILL_DIR/scripts/extract-theme.mjs --list \
              $SKILL_DIR/themes/all-themes-meta.json
...
要换一个主题，重新展开 tokens.css 即可：
  node $SKILL_DIR/scripts/extract-theme.mjs \
    $SKILL_DIR/themes/all-themes.css <id> src/styles/tokens.css
  # 列全部主题：node $SKILL_DIR/scripts/extract-theme.mjs --list \
  #   $SKILL_DIR/themes/all-themes-meta.json
