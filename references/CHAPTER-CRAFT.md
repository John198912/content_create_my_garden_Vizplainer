...
## Vizplainer 借鉴的六条进阶手法

> 这六条是从 Vizplainer（顶级 AIGC 解释类视频）的视觉设计拆出来的
> 可复用手法。**不是硬规**，是提质的手柄 —— 哪一条能让这一步更
> 像一段精制视频就用哪条。

### B1 · 层级三板斧（尺寸 × 字重 × 稀缺色）

一屏上的主次关系要**狠**，三个旋钮同时拉开：

- **尺寸跨度 ≥ 5:1**：hero 元素（标题 / 数字）跟辅助文字拉到至少
  5 倍差距。hero 120px 配合辅助 20px，别都是 40~60px 的"中间号"。
- **字重跳变 700↔300**：重要的用 display / 黑体，次要的直接跳到
  细体（`font-weight: 300`，Inter 200 已在 fonts.css 里备好）。中间
  字重（500/600）满屏 = 没层级。
- **强调色稀缺**：accent 只点一两处（见 B4），其余靠尺寸 + 字重
  做层级，不靠颜色。

> 文字色现在有 **5 档**：`--text` / `--text-2` / `--text-mute` /
> `--text-faint` / `--text-ghost`（最淡，用于背景脚手架 / 水印数字 /
> 未选中态）。用足这 5 档才有细腻的层级。

### B2 · 拟物 UI 演示（把抽象过程装进"真实界面"）

Vizplainer 最狠的手法：不画抽象框图，而是把过程装进一个**看起来
是真实软件的容器**，让观众以为"这东西真的在跑"：

- **`.window-frame`**（animations.css §13）—— macOS 风窗口（3 点 +
  标题栏 + 文件名槽 + body），装截图 / 代码 / 对话 / 图表。token
  驱动，明暗通吃。默认单色 3 点，`.is-mac` 才上真彩。
- **模拟终端**：mono 代码 + `.typing-dots` / `.spinner` + `.status-dot.is-live`。
- **模拟编辑器**：行号 + 语法色（走 token）+ 光标闪烁。
- **模拟 AI 对话**：`.sys-label`（机器）vs `.human-note`（人）分语境。
- **模拟文件树 / 时间线**：用 `.cascade` / `.rise-step` 逐项出。

参考骨架：[`EXAMPLES/ui-mockup-scene/`](EXAMPLES/ui-mockup-scene/)。

### B3 · 真实内容 > 占位（仅限文本）

演示里的**文本**要用**真的**：真文件名（`clean.py` 不是 `file1.py`）、
真命令、真字段名、真业务数据（或编得像真业务）。`lorem ipsum` /
`data1, data2` / `示例文本` 一眼出戏。

> ⚠️ **边界**：这条只管**文本**。**图片 / 截图仍用 placeholder 占位卡**
> （反 AI 味铁律：不编假图 / 假 logo）。两者不矛盾：文本可以真（或
> 像真的业务虚构），图像资产没有就老实留位。
...
- **一屏只出 1~2 处满格 accent**，其余全是 ink（text 系）。
- accent 优先给**"状态 / 结果 / 赢家"**：UI 里的 RUNNING / DONE 状态点、
  对比里赢的那根柱、论点里点睛的那个词。
- **正文 / 代码 / 表格不上 accent** —— 那是 text-2 的活。
- 渐变 / 光晕需第二色时用 `--accent-2`（只作渐变 / glow 伙伴，
  **不当独立饱和色用**）。

### B5 · 密度反比（留白处越空，信息处越实）

Vizplainer 的呼吸感来自**密度反差**：

- 文字屏（论点 / 金句）敢大留白，一屏就一句话。
- 演示屏（界面 / 图表 / 代码）敢填满信息。
- **两种屏交替**，反差本身就是节奏。都留白 = 碎；都填满 = 累。
- 同一屏内也可用：非对称分屏（33% 文字 / 67% 视觉）—— 参考
  [`EXAMPLES/asymmetric-split/`](EXAMPLES/asymmetric-split/)。

### B6 · 流程类→优先 UI 轮播，不是箭头框图

口播在讲"输入→处理→产出"这种**过程 / 流程**时：

- **优先**：一个 `.window-frame`，用 `.carousel-slide` 在三态间**前翻
  切换**（同一个容器换 body）—— 有"在用"的感觉。
- **避免**：三个框并排 + 箭头连接 —— 那是 PPT 流程图，静态、无叙事。
- **每态 = 1 step**（流程禁一次三联画，同逐步揭示原则）。

参考骨架：[`EXAMPLES/ui-mockup-scene/`](EXAMPLES/ui-mockup-scene/)。
竞品对比请独立成章：[`EXAMPLES/comparison-frame/`](EXAMPLES/comparison-frame/)。
...
- **颜色**：`--shell` / `--surface` / `--surface-2` / `--surface-3` /
  `--text` / `--text-2` / `--text-mute` / `--text-faint` / `--text-ghost` /
  `--rule` / `--accent` / `--accent-soft` / `--accent-glow` /
  `--accent-2`（部分主题有，只作渐变 / glow 伙伴）——
  **禁硬编码 hex / rgb / 颜色名**
- **可复用原语**（可选，走 token，明暗通吃）：`.window-frame`（拟物窗口）
  / `.sys-label`（机器语境 mono）/ `.human-note`（人语境 sans）/
  v2 动效词汇（`.cascade` / `.carousel-slide` / `.status-dot` / `.spinner`
  等，见 animations.css）
