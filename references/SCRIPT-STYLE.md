...
## 整片叙事弧线（结构层面 · Vizplainer 借鉴）

前面 8 条管的是**单句 / 单段**怎么说，这一节管的是**整片怎么排**。
Vizplainer 的解说页之所以"看着像一条完整的论证"而不是"一堆并列卡
片"，靠的就是下面三条叙事结构纪律。它们落在 `script.md` 的**段落顺序
+ 节拍密度**上，不改单句风格。

### 一、问题式 hook：先抛痛点 / 张力，再给答案

原则 4 的三种通用钩子（悬念 / 反差 / 利益）之外，**优先考虑"问题式
hook"** —— 开篇先把观众"为什么要听"的那个**问题 / 痛点 / 旧办法的别
扭**摆出来，让后面的内容变成"这个问题的答案"。这样整片有一条天然的
"问题 → 解法"主线，而不是平铺直叙报参数。

| ❌ 平铺开场 | ✓ 问题式 hook |
|---|---|
| "这是一个新的 AI 出图工具，支持很多功能。" | "以前做一张海报，得开 PS、抠图、调色，折腾一下午。现在呢？" |
| "我们这套系统有三个模块。" | "你有没有过这种事 —— 同一份数据，三个人跑出三个结果？" |
| "今天讲讲量化因子模型。" | "为什么有人用同样的数据，却总比你多赚一截？" |

> 问题式 hook **不是假共情**。"以前做海报得折腾一下午"是具体痛点
> （有场景、有信息量）；"我知道你最近肯定很纠结做图"是假共情（空泛
> + 上来装熟，见去 AI 味第一类）。区别还是那句：**有没有具体场景**。

### 二、密度反比弧线：开篇钩子要轻，主体要密，收尾再收

一整片的**信息密度不是均匀的**，而是一条弧线：
...
铺垫（中 · 把问题讲清楚、给一个具体场景）
   ▼
主体（密 · 解法 / 演示 / 对比 / 数据，最多干货）
   ▼
收尾（轻 · 一句话落点 + 具体 CTA）
```

开篇就堆满参数 = 观众还没"上钩"就被淹。主体阶段（通常是中间几章）
才是放最多硬信息（演示、数据、对比）的地方。这条弧线对应到
[`OUTLINE-FORMAT.md`](OUTLINE-FORMAT.md) 就是**首章 step 少而轻、中间
章 step 多而密**。

> 这与原则 7「不堆结构词」不冲突：弧线是**内容密度**的安排，不是
> 嘴上喊"首先 / 接下来进入重点"。结构感从密度变化来，不从指令来。

### 三、对比 / 反方单独成段，不要塞进一句话

凡是"A vs B"、"旧办法 vs 新办法"、"别人以为 X 其实 Y"、"它的短板"
这类**对比 / 反方**内容，在口播稿里要**单独切成一段（一个 `---`）**，
不要压缩进一句带过。原因：对比是观众最容易记住、最有说服力的结
构，给它一个独立节拍，后面 outline 才能把它实现成一个**独立对比章
节 / 对比画面**（见 OUTLINE-FORMAT「对比 / 反方独立成章」与
CHAPTER-CRAFT 的 `comparison-frame`）。
...
但你可能会说，快不代表好 —— 我一开始也这么想。

---

直到我把它和我手画的那版摆一起：构图、配色、连中文字都对，我手画
那版反而显得糙。
```

> 三段把"它快 → 你的质疑 → 用对比打消质疑"拆开，每段一个节拍。
> 比"它又快又好质量也不输手画"这种一句话糊过去强得多。
