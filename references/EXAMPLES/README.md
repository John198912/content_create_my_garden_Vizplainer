...
| [`ui-mockup-scene/`](ui-mockup-scene/) | **真实界面演示型**（Vizplainer #1+#2）—— 一个 `.window-frame` 装产品界面，输入→处理→产出 三态轮播前翻 | `chapter.tsx` + `chapter.css` |
| [`asymmetric-split/`](asymmetric-split/) | **非对称分屏型**（留白哲学）—— 33% 文字 / 67% 视觉，密度反比 | `chapter.tsx` + `chapter.css` |
| [`comparison-frame/`](comparison-frame/) | **竞品对比型**（Vizplainer #8）—— 独立一屏对比表，对照列灰态 + 我方列 accent 压轴，逐列揭示 | `chapter.tsx` + `chapter.css` |
