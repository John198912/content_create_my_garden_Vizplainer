...
const V2 = baseline.visualRegression.templates; // [newsroom-v2, bold-signal-v2, swiss-ikb-v2, viz-studio]
...
section('7. 强调色预算（--accent-2 仅渐变伙伴 / soft·glow 同色相）');
{
  for (const [theme, wantA2] of Object.entries(baseline.accentBudget.perThemeAccent2)) {
    ...
    // accent-soft / accent-glow 必须与 accent 同色相（仅 v2 themes 有这些档）
    if (V2.includes(theme)) {
      ...
      for (const [name, val] of [['--accent-soft', soft], ['--accent-glow', glow]]) {
        if (!val) continue;
        if (hueClose(hA, hueOf(val))) ok(`${theme} ${name} 与 --accent 同色相`);
        else bad(`${theme} ${name} 偏离 accent 色相`, `accent 色相 ${hA}° vs ${name} ${hueOf(val)}°`);
      }
    }
  }
}
