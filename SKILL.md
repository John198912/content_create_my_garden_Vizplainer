...
description: 把一篇文章或口播稿做成看起来像视频的点击驱动 16:9 网页演示，可选合成口播音频。流程：原始文章→口播稿+outline→用户对齐5件事→网页开发（逐章/顺序/并行）→可选音频合成（MiniMax/OpenAI TTS/ElevenLabs/Azure）。每次点击推进一个节拍，每步独占整屏。适用：用网页做视频、把口播稿/文章变成可交互解说、为B站/YouTube/视频号录屏教程、做有电影感的产品demo。触发词：做成视频/网页视频/动态PPT/口播稿/录屏教程/web video presentation。
---
