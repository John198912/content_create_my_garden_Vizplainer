...
/**
 * Auto-generate a storage key from the chapter structure so that any
 * structural change (add/remove/reorder chapters, change step counts)
 * automatically invalidates old persisted cursors. No more manual bumping.
 */
function deriveStorageKey(chapters: ChapterDef[]): string {
  const hash = chapters
    .map((c) => `${c.id}:${c.narrations.length}`)
    .join("|");
  // Simple djb2 hash — deterministic, fast, and collision-resistant enough
  let h = 5381;
  for (let i = 0; i < hash.length; i++) {
    h = (h * 33) ^ hash.charCodeAt(i);
  }
  return `presentation-cursor-${(h >>> 0).toString(36)}`;
}
...
export function useStepper(chapters: ChapterDef[]): StepperState {
  const storageKey = useMemo(() => deriveStorageKey(chapters), [chapters]);
  const isCleanRun =
    typeof window !== "undefined" &&
    new URLSearchParams(window.location.search).get("clean") === "1";
  const deterministicCleanRunRef = useRef(isCleanRun);

  const [cursor, setCursor] = useState<Cursor>(() => {
    const fallback = { chapter: 0, step: 0 };
    if (typeof window === "undefined") return fallback;
    if (isCleanRun) return fallback;
    try {
  ...
  useEffect(() => {
    if (deterministicCleanRunRef.current) return;
    try {
