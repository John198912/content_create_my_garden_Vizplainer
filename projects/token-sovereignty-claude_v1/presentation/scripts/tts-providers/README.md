...
| `edge-tts.sh` | Microsoft Edge TTS | 无（免费、需联网） | pip install edge-tts；无需 API key，但不是离线服务 |

内置三个 provider —— 其它后端的代码片段在下面，复制到 `tts-providers/<name>.sh` 即可启用。
---
...
7. **API 长度上限**：单段大多数 API 都有上限。V2.1 源单元是 1–3 句完整语义段；超限时按自然停顿拆 segment，再重新拼接章节 master，不按 Beat 机械切碎。
